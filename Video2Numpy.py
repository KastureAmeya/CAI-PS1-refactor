...
if __name__=="__main__":
    src_path = 'videos.json' #path of json file. [clip (.mp4):label]
    target_path = './Data_DIR/npy_dir'
    filename = 'videos_npy.json'
    os.makedirs(target_path,exist_ok=True)

    annotations = json.load(open('videos.json')) # [clip(.mp4):label]
    ...
    for source, label in tqdm(annotations.items()):
        if(label=="violent" or label =="non-violent"):
            save_path = os.path.join(target_path, os.path.basename(source).replace('.mp4','.npy'))
